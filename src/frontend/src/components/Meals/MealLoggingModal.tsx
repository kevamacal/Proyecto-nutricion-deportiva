...
export const MealLoggingModal: React.FC<MealLoggingModalProps> = ({
  ...
  const calcCal = selectedFood?.nutrition ? Math.round((quantity / 100) * selectedFood.nutrition.calories_kcal) : 0;
  const calcProt = selectedFood?.nutrition ? Math.round((quantity / 100) * selectedFood.nutrition.protein_g) : 0;
  const calcCarbs = selectedFood?.nutrition ? Math.round((quantity / 100) * selectedFood.nutrition.carbohydrates_g) : 0;
  const calcFat = selectedFood?.nutrition ? Math.round((quantity / 100) * selectedFood.nutrition.fat_g) : 0;
  ...
  return (
    <>
      <BaseModal
        isOpen={isOpen}
        onClose={onClose}
        title="Registrar Comida / Ingesta Calórica"
